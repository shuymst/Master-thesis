...
そこで価値関数$v_\pi$の推定値$V$を用いる．
さらに$G_t = \Sigma_{k=0}^{T} R_{t+k+1}$を$\Sigma_{k=0}^{n-1} R_{t+k+1} + V(S_{t+n})$によって推定する．



このように学習した価値関数をactor-criticな手法と呼ばれる．
...
Stochastic MuZeroは環境のダイナミクスモデルをデータから学習し， これを用いてモンテカルロ木探索~(MCTS)~によるプランニングを行う．
ここではStochastic MuZeroのMCTSの方法， および方策・価値ニューラルネットワークの学習方法について説明する．
環境のダイナミクスモデルの学習方法については論文~\cite{StochasticMuZero}の$4$節を参照されたい．

\subsubsection{Stochastic MuZeroのモンテカルロ木探索}
Stochastic MuZeroはMCTSにおいて， \textit{decisionノード}と\textit{chanceノード}という2種類のノードから成る探索木を用いる．
decisionノードからchanceノードへの遷移は決定的で， chanceノードからdecisionノードへの遷移は確率的に行われる．
すなわち2048の文脈では， decisionノードはプレイヤが行動を選択する状態に対応し， chanceノードはafterstateに対応する．
よって本稿では分かりやすさのため， decisionノードを状態ノード， chanceノードをafterstateと呼ぶことにする．
そのためdecisionノードの子はchanceノードで， chanceノードの子はdecisionノードである．

探索木中のそれぞれのノード$s$は， ${N(s), W(s), Q(s), P(s)}$という$4$つの統計量を管理する．
\begin{itemize}
  \item $N(s) \cdots$探索中に$s$を訪問した回数
  \item $W(s) \cdots$ $s$を根とする部分木の価値の推定値の合計
  \item $Q(s) \cdots$ $s$を根とする部分木の価値の推定値の平均~($W(s,a) / N(s,a)$)
